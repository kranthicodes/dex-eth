...
contract("Dex", (accounts) => {
  ...
  it("should NOT deposit token if token does not exist", async () => {
    const amount = web3.utils.toWei("100");
    await expectRevert(
      dex.deposit(amount, web3.utils.fromAscii("SOMETOKEN"), { from: trader1 }),
  ...
  it("should NOT withdraw tokens if token does not exist", async () => {
    const amount = web3.utils.toWei("100");
    await expectRevert(
      dex.withdraw(amount, web3.utils.fromAscii("SOMETOKEN"), {
        from: trader1,
      }),
      "this token does not exist"
    );
  ...
  it("should create limit order", async () => {
    ...
    const buyOrders1 = await dex.getOrders(REP, SIDE.BUY);
    const sellOrders1 = await dex.getOrders(REP, SIDE.SELL);
    assert(buyOrders1.length === 2);
    assert(buyOrders1[0].trader === trader2);
    assert(buyOrders1[1].trader === trader1);
    assert(sellOrders1.length === 0);
    ...
    assert(buyOrders2[2].trader === trader2);
    assert(buyOrders2[2].price === "9");
    assert(sellOrders2.length === 0);
  });
  it("should NOT create limit order if token doesnt exist", async () => {
    await expectRevert(
      dex.createLimitOrder(
        web3.utils.fromAscii("SOMETOKEN"),
  ...
  it("should NOT create market order if token doesnt exist", async () => {
    await expectRevert(
